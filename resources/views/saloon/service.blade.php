...
@section('main')
@push('title')
<title>{{ trans('messages.view_service_lang', [], session('locale')) }}</title>
@endpush
...
<main class="min-h-screen bg-surface" x-data="{ edit: false }">
    <header class="bg-[#f9f9f9] dark:bg-[#1a1c1c] flex justify-between items-center w-full px-8 py-4 sticky top-0 z-10">
        <div class="flex items-center gap-6">
            <button
                type="button"
                @click="$store.modals.service = true; edit = false"
                ...
                {{ trans('messages.add_service_lang', [], session('locale')) }}
            </button>
        </div>
    </header>
        ...
        <section class="bg-surface-container-lowest rounded-xl editorial-shadow overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-surface-container-low">
                            ...
                            <th class="px-6 py-5 text-[11px] font-bold uppercase tracking-[0.1em] text-primary">{{ trans('messages.service_name_lang', [], session('locale')) }}</th>
                            <th class="px-6 py-5 text-[11px] font-bold uppercase tracking-[0.1em] text-primary">{{ trans('messages.service_price_lang', [], session('locale')) }}</th> 
            ...
            <div class="px-8 py-5 flex items-center justify-between border-t border-surface" id="data-pagination">
                <span class="text-xs text-on-surface-variant font-medium">
                    Showing {{ $service->firstItem() ?? 0 }} to {{ $service->lastItem() ?? 0 }} of {{ $service->total() }} service
                </span>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ $service->previousPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center hover:bg-surface-container-low transition-colors text-on-surface-variant dress-page-link">
                            <span class="material-symbols-outlined text-sm">chevron_left</span>
                        </a>
                    @endif
                    ...
                    @if ($service->hasMorePages())
                        <a href="{{ $service->nextPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center hover:bg-surface-container-low transition-colors text-on-surface-variant dress-page-link">
    ...
    <div
        x-show="$store.modals.service"
        x-cloak
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
        id="add_service_modal">

        <div
            @click.away="$store.modals.service = false; edit = false"
            class="bg-white rounded-2xl shadow-xl w-full max-w-4xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto">

            <div class="flex justify-between items-start mb-6">
                <h1 class="text-xl sm:text-2xl font-bold">
                    <span x-text="edit ? '{{ trans('messages.edit_service_lang', [], session('locale')) }}' : '{{ trans('messages.add_service_lang', [], session('locale')) }}'"></span>
                </h1>

                <button
                    type="button"
                    @click="$store.modals.service = false; edit = false"
            ...
            <form id="service_form" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-base font-medium mb-2">
                            {{ trans('messages.service_price_lang', [], session('locale')) }}
                            <span class="text-red-500">*</span>
                        </label>
                        <input
                            type="number"
...
@include('custom_js.salonservice_js')
@endsection