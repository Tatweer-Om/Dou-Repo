...
@section('main')
@push('title')
<title>{{ trans('messages.channel_stock_audit', [], session('locale')) }} - {{ trans('messages.audit_details', [], session('locale')) }}</title>
@endpush
...
<main class="flex-1 p-8 bg-background-light dark:bg-background-dark overflow-y-auto">
    <div class="max-w-[98%] mx-auto">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">{{ trans('messages.channel_audit_description', [], session('locale')) }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-6 border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input id="searchInput" type="text" placeholder="{{ trans('messages.search_stock_audit', [], session('locale')) }}" class="w-full h-10 rounded-lg border border-gray-300 px-4 pr-10 focus:ring-2 focus:ring-primary/50 focus:border-primary transition" />
                        <span class="material-symbols-outlined absolute right-3 top-2.5 text-gray-400">search</span>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2 text-gray-700">{{ trans('messages.from_date', [], session('locale')) ?: 'From Date' }}</label>
                    <input id="fromDate" type="date" class="w-full h-10 rounded-lg border border-gray-300 px-4 focus:ring-2 focus:ring-primary/50 focus:border-primary transition" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2 text-gray-700">{{ trans('messages.to_date', [], session('locale')) ?: 'To Date' }}</label>
                    <input id="toDate" type="date" class="w-full h-10 rounded-lg border border-gray-300 px-4 focus:ring-2 focus:ring-primary/50 focus:border-primary transition" />
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-blue-800">
                        {{ trans('messages.remaining_quantity', [], session('locale')) ?: 'Remaining Quantity' }}: <span id="remainingQtyValue" class="text-xl font-bold">0</span>
                    </p>
...
@include('custom_js.channel_audit_js')
@endsection
