<script>
    ...
    $.get("{{ url('expense-categories/list') }}?page=" + page, function(res) {
        ...
        for (let i = 1; i <= res.last_page; i++) {
            ...
            <li class="px-3 py-1 rounded-full ${res.current_page == i ? 'bg-[var(--primary-color)] text-white' : 'bg-gray-200 hover:bg-gray-300'}">
                <a href="{{ url('expense-categories/list') }}?page=${i}">${i}</a>
            </li>
            `;
        }
...
$(document).on('click', '#pagination a', function(e) {
    ...
    if (href && href !== '#') {
        ...
        if (page) loadExpenseCategories(page);
    }
});
        ...
        $('#search_expense_category').on('keyup', function() {
        ...
        $('#expense_category_form').submit(function(e) {
            e.preventDefault();
            let id = $('#expense_category_id').val();
            let category_name = $('#expense_category_name').val().trim();
            let notes = $('#expense_category_notes').val().trim();
            ...
            let url = id ? `{{ url('expense-categories') }}/${id}` : "{{ url('expense-categories') }}";
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.get("{{ url('expense-categories') }}/" + id, function(category) {
                $('#expense_category_id').val(category.id);
                $('#expense_category_name').val(category.category_name);
                $('#expense_category_notes').val(category.notes || '');
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(data) {
                            loadExpenseCategories(); // reload table
                            Swal.fire(
