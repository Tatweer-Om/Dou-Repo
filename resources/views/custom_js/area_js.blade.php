<script>
    $(document).ready(function() {
       ...
       function loadAreas(page = 1) {
    $.get("{{ url('areas/list') }}?page=" + page, function(res) {

        // ---- Table Rows ----
        let rows = '';
        $.each(res.data, function(i, area) {
            const notesPreview = area.notes ? 
                (area.notes.length > 50 ? area.notes.substring(0, 50) + '...' : area.notes) : 
                '-';
            
            rows += `
            <tr class="hover:bg-pink-50/50 transition-colors" data-id="${area.id}">
                <td class="px-4 sm:px-6 py-5 text-[var(--text-primary)]">${area.area_name_en || '-'}</td>
                <td class="px-4 sm:px-6 py-5 text-[var(--text-primary)]">${area.area_name_ar || '-'}</td>
        ...
        for (let i = 1; i <= res.last_page; i++) {
            ...
            <li class="px-3 py-1 rounded-full ${res.current_page == i ? ' text-white' : 'bg-gray-200 hover:bg-gray-300'}">
                <a href="{{ url('areas/list') }}?page=${i}">${i}</a>
            </li>
            `;
        }
...
$(document).on('click', '#pagination a', function(e) {
    ...
    if (href && href !== '#') {
        ...
        $('#area_form').submit(function(e) {
            e.preventDefault();
            let id = $('#area_id').val();
            let area_name_en = $('#area_name_en').val().trim();
            let area_name_ar = $('#area_name_ar').val().trim();
            let notes = $('#notes').val().trim();

            // Simple validation
            if (!area_name_en) {
                show_notification('error', '<?= trans("messages.enter_area_name_en", [], session("locale")) ?>');
                return;
            }

            let url = id ? `{{ url('areas') }}/${id}` : "{{ url('areas') }}";
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.get("{{ url('areas') }}/" + id, function(area) {
                $('#area_id').val(area.id);
                $('#area_name_en').val(area.area_name_en || '');
                $('#area_name_ar').val(area.area_name_ar || '');
                $('#notes').val(area.notes || '');
