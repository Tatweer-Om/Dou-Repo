...
$(document).ready(function() {
    ...
    function loadAuditData(page = 1) {
        ...
        $.get("{{ url('stock/channel-audit/list') }}?" + params.toString(), function(res) {
            if (!res.success) {
            ...
            if (res.remaining_quantity !== null && res.remaining_quantity !== undefined) {
                ...
                if (Array.isArray(res.remaining_by_size) && res.remaining_by_size.length > 0) {
                    const parts = res.remaining_by_size.filter(x => x && x.size !== undefined && x.quantity !== undefined).map(x => `${x.size}: ${x.quantity}`);
                    $('#remainingBySize').text(parts.join(' | '));
                } else $('#remainingBySize').text('');
            } else {
            ...
            if (res.data && res.data.length > 0) {
                $.each(res.data, function(i, item) {
                    ...
                    const opClass = item.operation_type === 'sold' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800';
                    rows += `
                        <tr class="hover:bg-gray-50 border-b">
                            ...
                            <td class="px-4 py-3 border text-right text-xs"><span class="px-1 rounded text-xs font-medium ${opClass}">${item.operation_label}</span></td>
    ...
    function renderPagination(currentPage, lastPage, total, perPage) {
        ...
        if (total > 0) {
            ...
            pagination += `<li class="flex"><a href="#" data-page="${currentPage - 1}" class="px-3 py-1 border rounded-lg mx-1 text-sm transition ${currentPage == 1 ? 'opacity-40 cursor-not-allowed bg-gray-200' : 'bg-white hover:bg-gray-100'}">&laquo;</a></li>`;
            let startPage = Math.max(1, currentPage - 2);
            let endPage = Math.min(lastPage, currentPage + 2);
            if (startPage > 1) {
                pagination += `<li><a href="#" data-page="1" class="px-4 py-1 mx-1 text-sm font-medium border rounded-lg transition bg-white hover:bg-gray-100">1</a></li>`;
                if (startPage > 2) pagination += `<li class="px-2 text-gray-500 flex items-center">...</li>`;
            }
            for (let i = startPage; i <= endPage; i++) {
                pagination += `<li><a href="#" data-page="${i}" class="px-4 py-1 mx-1 text-sm font-medium border rounded-lg transition ${currentPage == i ? 'bg-[var(--primary-color)] text-white border-[var(--primary-color)] shadow-md' : 'bg-white hover:bg-gray-100'}">${i}</a></li>`;
            }
            if (endPage < lastPage) {
                if (endPage < lastPage - 1) pagination += `<li class="px-2 text-gray-500 flex items-center">...</li>`;
                pagination += `<li><a href="#" data-page="${lastPage}" class="px-4 py-1 mx-1 text-sm font-medium border rounded-lg transition bg-white hover:bg-gray-100">${lastPage}</a></li>`;
            }
            pagination += `<li class="flex"><a href="#" data-page="${currentPage + 1}" class="px-3 py-1 border rounded-lg mx-1 text-sm transition ${currentPage == lastPage ? 'opacity-40 cursor-not-allowed bg-gray-200' : 'bg-white hover:bg-gray-100'}">&raquo;</a></li>`;
        ...
        $(document).off('click', '#pagination a').on('click', '#pagination a', function(e) {
            ...
            if (page && page > 0 && page <= lastPage) loadAuditData(page);
        });
    }
